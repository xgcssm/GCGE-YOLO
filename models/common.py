...
class CoordAtt(nn.Module):
    ...
    def forward(self, x):
        ...
        outputsz_h = np.array([h, 1])
        outputsz_w = np.array([1, w])

        stridesz_h = np.floor(inputsz / outputsz_h).astype(np.int32)
        stridesz_w = np.floor(inputsz / outputsz_w).astype(np.int32)

        kernelsz_h = inputsz - (outputsz_h - 1) * stridesz_h
        kernelsz_w = inputsz - (outputsz_w - 1) * stridesz_w

        pool_h = nn.AvgPool2d(kernel_size=list(kernelsz_h), stride=list(stridesz_h))
        pool_w = nn.AvgPool2d(kernel_size=list(kernelsz_w), stride=list(stridesz_w))
